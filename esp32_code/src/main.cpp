...
void loop()
{
    ...
    {
        if (last_pb1 == HIGH && current_pb1 == LOW) { publish_joint(1); last_press_time = now; }
        else if (last_pb2 == HIGH && current_pb2 == LOW) { publish_joint(2); last_press_time = now; }
        else if (last_pb3 == HIGH && current_pb3 == LOW) { publish_joint(3); last_press_time = now; }
        else if (last_pb4 == HIGH && current_pb4 == LOW) { publish_joint(4); last_press_time = now; }
        else if (last_pb5 == HIGH && current_pb5 == LOW) { publish_joint(5); last_press_time = now; }
        else if (last_pb6 == HIGH && current_pb6 == LOW) { publish_joint(6); last_press_time = now; }
    }
